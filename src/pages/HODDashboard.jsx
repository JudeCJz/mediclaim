...
const recruiterApp = initializeApp(firebaseConfig, "HOD_MASTER_V2");
...
const HODDashboard = () => {
    ...
    const processBulk = async () => {
        ...
        alert(`Successfully registered ${rows.length} personnel accounts. Logins are now active using email as password.`); setBulkData(""); setIsProcessing(false);
    };
    ...
    // Filter submissions to only show those belonging to faculty in this department
    const enrolledIds = submissions.map(s => s.userId);
    const enrolledInDept = faculty.filter(f => enrolledIds.includes(f.id));

    const stats = {
        total: faculty.length,
        enrolled: enrolledInDept.length,
        pending: Math.max(0, faculty.length - enrolledInDept.length)
    };

    return (
        <div style={{ maxWidth: '1600px', margin: '0 auto' }}>
            <header style={{ marginBottom: '3rem', borderLeft: '12px solid var(--primary)', paddingLeft: '2rem' }}>
                <h1 style={{ fontSize: '3rem', fontWeight: 900 }}>Department Overview</h1>
                <p style={{ color: 'var(--text-muted)', fontWeight: 700 }}>Welcome, {user?.name || 'HOD'}</p>
            </header>


            {activeTab === 'overview' && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                    <div className="glass-panel" style={{ padding: '2.5rem', borderLeft: '12px solid var(--primary)' }}>
                        <h3 style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '1rem', fontWeight: 900 }}>Total Personnel</h3>
                        <div style={{ fontSize: '4.5rem', fontWeight: 900, lineHeight: 1 }}>{stats.total}</div>
                        <div style={{ fontSize: '0.7rem', fontWeight: 800, marginTop: '1rem', opacity: 0.6 }}>[ REGISTERED_ACCOUNTS ]</div>
                    </div>
                    <div className="glass-panel" style={{ padding: '2.5rem', borderLeft: '12px solid #22c55e' }}>
                        <h3 style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '1rem', fontWeight: 900 }}>Enrolled Faculty</h3>
                        <div style={{ fontSize: '4.5rem', fontWeight: 900, lineHeight: 1, color: '#22c55e' }}>{stats.enrolled}</div>
                        <div style={{ fontSize: '0.7rem', fontWeight: 800, marginTop: '1rem', opacity: 0.6 }}>[ CHOICE_SUBMITTED ]</div>
                    </div>
                    <div className="glass-panel" style={{ padding: '2.5rem', borderLeft: '12px solid #ef4444' }}>
                        <h3 style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '1rem', fontWeight: 900 }}>Pending Action</h3>
                        <div style={{ fontSize: '4.5rem', fontWeight: 900, lineHeight: 1, color: '#ef4444' }}>{stats.pending}</div>
                        <div style={{ fontSize: '0.7rem', fontWeight: 800, marginTop: '1rem', opacity: 0.6 }}>[ AWAITING_ENROLLMENT ]</div>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'recruit' && (
                <div className="glass-panel" style={{ padding: '3rem' }}>
                    ...
                    <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                        ...
                        <button className="btn btn-primary" onClick={processBulk} disabled={isProcessing || !bulkData}>
                    ...
                    <div style={{ marginTop: '5rem', borderTop: '2px dashed var(--border-glass)', paddingTop: '3rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
                            ...
                            <div style={{ position: 'relative', width: '100%', maxWidth: '350px' }}>
                                ...
                                <input 
                                    className="glass-panel" 
                                    style={{ width: '100%', padding: '0.6rem 1rem 0.6rem 2.8rem', fontSize: '0.8rem', fontWeight: 600 }} 
                            ...
                            <tbody>
                                {faculty.filter(f => 
                                    ...
                                    <tr key={f.id}>
                                        ...
                                        <td style={{ fontWeight: 700 }}>{f.department || 'General'}</td>
                    ...
                    <div className="glass-panel" style={{ width: '90%', maxWidth: '700px', padding: '3rem', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <div style={{ borderTop: '2px dashed var(--border-glass)', paddingTop: '2.5rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
                                ...
                                <button className="btn btn-primary" style={{ height: '54px', padding: '0 2rem' }} onClick={addPolicyToFY}>ADD TIER</button>
                            </div>
